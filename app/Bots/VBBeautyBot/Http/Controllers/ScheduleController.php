...
use App\Bots\VBBeautyBot\Http\Requests\ScheduleStoreRequest;
...
class ScheduleController extends Controller
{
    public function index()
    {
            ...
            if (!$schedule->appointments->isEmpty()) {
                ...
                foreach ($schedule->appointments as $appointment) {
                    ...
                    $events[] = [
                        ...
                        'url' => route('VBBeautyBot.appointment.index', ['date' => $appointment->schedule->date->format('Y-m-d'), 'user' => $appointment->schedule->user_id]),
                    ];
                }
            }
            else {
		...
		return view('VBBeautyBot::schedule.index', compact(
            'events'
        ));
    }
