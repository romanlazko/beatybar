            ...
            <div class="w-full sm:w-1/3 space-y-3">
                @if (auth()->user()->isAdmin())
                    <x-telegram::white-block>
                        <form action="{{ route('valeri_beautybar_bot.schedule.index') }}" id="userForm">
                            <div class="">
                ...
                @if (request('user', auth()->user()->id))
                    <x-telegram::white-block>
                        <form method="post" action="{{ route('valeri_beautybar_bot.schedule.store') }}">
                            @csrf
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                var calendar = new FullCalendar.Calendar(calendarEl, {
                    ...
                    eventClick: function (eventClickInfo) {
                        ...
                        editEventModalForm.setAttribute('action', "{{ route('valeri_beautybar_bot.schedule.update', '') }}/"+eventClickInfo.event.id);
                        editEventTerm.setAttribute('value', eventClickInfo.event.startStr.split('T')[1].split('+')[0]);

                        deleteEventModalForm.setAttribute('action', "{{ route('valeri_beautybar_bot.schedule.destroy', '') }}/"+eventClickInfo.event.id);
                    },
                    firstDay: 1,
