<div class="block">
    ...
    @if ($appointment->status === 'new')
        <form method="POST" action="{{ route('valeri_beautybar_bot.appointment.update', $appointment) }}" class="bg-gray-200 p-3 my-2 rounded-md space-y-2 form hidden">
            @csrf
            @method('patch')
            ...
            <div>
                <x-telegram::form.select name="schedule" class="w-full">
                    @forelse (
                        App\Bots\valeri_beautybar_bot\Models\Schedule::with('appointment')
                            ->orderBy('date', 'asc')
